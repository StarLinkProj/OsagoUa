...
SmFaq.slider = function (inputId, direction) {
	...
	rerunFunction = true;
	contHeight = contentObj.offsetHeight + 10;
...
SmFaq.sendform = function (f) {
	...
	if (jQuery("input[name='created_by']").val() == "") {
		jQuery(".faq-name-error").show();
	} else if (jQuery("input[name='created_by']").val() != "") {
		jQuery(".faq-name-error").hide();
	}
	
	var reg = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,6})+$/;
	var mailVal = jQuery("input[name='created_by_email']").val();
	if (mailVal == "" || !reg.test(mailVal)) {
		jQuery(".faq-mail-error").show();
	} else if (mailVal != "" && reg.test(mailVal)) {
		jQuery(".faq-mail-error").hide();
	}
	
	var questionVal = jQuery("textarea[name='question']").val();
	if (questionVal == "" || questionVal.length < 10) {
		jQuery(".faq-question-error").show();
	} else if (questionVal != "" && questionVal.length >= 10) {
		jQuery(".faq-question-error").hide();
	}
