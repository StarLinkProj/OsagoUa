...
<div class="item-page<?php echo $this->pageclass_sfx; ?>" itemscope itemtype="http://schema.org/Article">
	...
	<?php if (preg_match('/\/news/', $_SERVER['REQUEST_URI'])) {
			$rusMonth = array("Января", "Февраля", "Марта", "Апреля", "Мая", "Июня", "Июля", "Августа", "Сентября", "Октября", "Ноября", "Декабря");
			
			$timestamp = strtotime($this->item->created);
			
			echo "<div class='item-time'>" . date("d", $timestamp) . " " . $rusMonth[date("m", $timestamp) - 1] . " " . date("Y", $timestamp) . "</div>"; 
		}
	?>
	<?php if (!$this->print) : ?>
